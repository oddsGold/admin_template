...
const createUserSchema = (isPasswordRequired: boolean) =>
  z
    .object({
...
const UserForm = React.forwardRef<UserFormRef, UserFormProps>(
  (
    { current, defaultCurrent, handleSubmit: onSubmit, roles, backLinkPath, password = false },
    ref
  ) => {
    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)} autoComplete="off" className="space-y-6">
        ...
        <div>
          ...
          />
          {errors.role && (
            <p className="mt-1.5 text-xs text-destructive text-error-500">{errors.role.message}</p>
          )}
        </div>
