...
>({
  ...
  const { selectedStatuses, removeFilter, setSelectedStatuses, clearFilters } = useFilters({
    ...
    <>
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="max-w-full overflow-x-auto">
          {isFilter && (
            <div className="flex flex-col gap-2 px-4 py-4 border-gray-100 dark:border-white/[0.05] rounded-t-xl sm:flex-row sm:items-center sm:justify-end">
              <div className="flex gap-2">
                {hasActiveFilters && renderSelectedFilters()}
                {selectedStatuses.selectedOptions.length > 0 && (
