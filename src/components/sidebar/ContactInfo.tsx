...
const ContactInfo: React.FC<ContactInfoProps> = ({ isExpanded, isHovered, isMobileOpen }) => {
  ...
  return (
    <div className={`px-4 dark:border-gray-700 ${!showContainer ? 'lg:hidden' : ''}`}>
      ...
      <div className="flex items-center gap-2 py-[12px]">
        ...
        {showContent && (
          <div>
            <p className="text-sm font-medium text-primary-default dark:text-white">Сулим Д.В</p>
          </div>
        )}
      </div>
    </div>
